    ...
    for instruction in body {
        match instruction {
            Instruction::Mov { assembly_type, src, dst } => {
                ...
                new_body.push(Instruction::Mov { assembly_type, src, dst });
            }
            Instruction::Unary { op, assembly_type, dst } => {
                let dst = match dst {
                    Operand::Pseudo(name) => {
                        fixup_pseudo(name, &mut stack, false, static_variables)
                    }
                ...
                new_body.push(Instruction::Unary { op, assembly_type, dst });
            }
            Instruction::Binary { op, assembly_type, src2, dst } => {
                ...
                new_body.push(Instruction::Binary { op, assembly_type, src2, dst });
            }
            ...
            Instruction::Cmp(assembly_type, lhs, rhs) => {
                if let Operand::Stack(_) | Operand::Data(_) = lhs {
                    if let Operand::Stack(_) | Operand::Data(_) = rhs {
                        ...
                        new_body.push(Instruction::Cmp(assembly_type.clone(), Operand::Register(Reg::R10), rhs));
                        continue;
                    }
                }
                if let Operand::Immediate { imm: _ } = rhs {
                    ...
                    new_body.push(Instruction::Cmp(assembly_type, lhs, Operand::Register(Reg::R10)));
                    continue;
                }
                new_body.push(instruction.clone());
            }
            Instruction::Binary { op, assembly_type, src2, dst } if op == BinaryOp::Mult => {
